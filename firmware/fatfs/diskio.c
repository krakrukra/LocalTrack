...
static void prevent_poweroff();
static void allow_poweroff();
...
)
{
  ...
  prevent_poweroff();//prevent poweroff during disk_write()
  ...
  allow_poweroff();//allow poweroff again
  return RES_OK;
}
...
{
  unsigned short i;
  prevent_poweroff();//prevent poweroff during mass_erase()
  ...
  wait_notbusy();//wait until last EB is erased
  allow_poweroff();//allow poweroff again
...
//provide power to the board even if mechanical switch is opened (use this during flash write operations)
static void prevent_poweroff()
{
  GPIOB->BSRR = (1<<13);//pull PB13 high
  GPIOB->MODER |= (1<<26);//configure PB13 as output
  while( !(GPIOB->IDR & (1<<13)) );//wait until PB13 is high
  
  return;
}

//allow mechanical switch to power the device off
static void allow_poweroff()
{
  GPIOB->BSRR = (1<<29);//pull PB13 low
  while(GPIOB->IDR & (1<<13));//wait until PB13 is low
  GPIOB->MODER &= ~(1<<26);//disconnect PB13
