...
void processMSDtransaction()
{
	  ...
	  if(MSDinfo.BytesLeft == MAXPACKET_MSD)
	    {
	      ...
	      USB->EP1R = (1<<8)|(1<<7)|(1<<0);//respond to OUT packets with NAK, ignore IN packets, clear CTR_RX flag
	      USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<7)|(1<<4)|(2<<0);//respond to IN packets with CSW, ingore OUT packets
	    }
	  
	  //if there are still data transactions left in this transfer
	  else
	    {
	      USB->EP1R = (1<<8)|(1<<7)|(1<<6)|(1<<0);//respond to OUT packets with ACK, ignore IN packets, clear CTR_RX flag
	      USB->EP2R = (1<<15)|(1<<8)|(1<<7)|(2<<0);//respond to IN packets with NAK, ingore OUT packets
  ...
  //in case of IN transaction
	...
	{
	  //if last data transaction was just completed
	  if( (BTABLE->COUNT2_TX == 0) || (BTABLE->COUNT2_RX == 0) )
	    {
	      sendCSW(0, 0x00, 0x00);//return good status
		...
		{
		  USB->EP1R = (1<<15)|(1<<8)|(1<<7)|(1<<0);//respond to OUT packets with NAK, ignore IN packets
		  USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<5)|(2<<0);//respond to IN packets with STALL, ignore OUT packets, clear CTR_TX flag
		}
	      //if device returned as much data as host expected
	      else
		{
		  USB->EP1R = (1<<15)|(1<<8)|(1<<7)|(1<<0);//respond with NAK to OUT packets, ignore IN packets
		  USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(2<<0);//respond to IN packets with CSW, ignore OUT packets, clear CTR_TX flag
		}
	    }
	  ...
	  //if there are still data transactions left in this transfer
	    ...
	    {
	      USB->EP1R = (1<<15)|(1<<8)|(1<<7)|(1<<0);//respond with NAK to OUT packets, ignore IN packets
	      USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(2<<0);//respond to IN packets with data, ignore OUT packets, clear CTR_TX flag
      ...
      else if(MSDinfo.MSDstage == STATUS)
	{
	  ...
	  USB->EP1R = (1<<15)|(1<<8)|(1<<7)|(1<<6)|(1<<0);//respond with ACK to OUT packets, ignore IN packets
	  USB->EP2R = (1<<15)|(1<<8)|(1<<4)|(2<<0);//respond to IN packets with NAK, ignore OUT packets, clear CTR_TX flag
	}
    }
...
static void processNewCBW()
{
  ...
  if(USB->EP1R & (1<<14)) {bufferOffset = BTABLE->ADDR1_TX; bytesReceived = BTABLE->COUNT1_TX & 0x03FF;}
  else                    {bufferOffset = BTABLE->ADDR1_RX; bytesReceived = BTABLE->COUNT1_RX & 0x03FF;}
  ...
  if(error)
    {
      ...
      USB->EP1R = (1<<13)|(1<<8)|(1<<7)|(1<<6)|(1<<0);//respond to OUT packets with STALL, ignore IN packets, clear CTR_RX flag
      USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<7)|(1<<5)|(1<<4)|(2<<0);//respond to IN packets with STALL, ignore OUT packets
      return;
    }
  ...
  switch( (MSDinfo.CBW).CBWCB[0] )
	...
	{ 
	  if( (MSDinfo.CBW).bmCBWFlags & (1<<7) )//if host wants to receive data
	    ...
	    {
	      USB->EP1R = (1<<13)|(1<<8)|(1<<7)|(1<<0);//respond to OUT packets with STALL, ignore IN packets, clear CTR_RX flag
	      USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<7)|(1<<4)|(2<<0);//respond to IN packets with CSW, ignore OUT packets
	    }
	}
...
static void processInquiryCommand_6()
{
  if( (MSDinfo.CBW).CBWCB[1] & 0x01 )//if EVPD is set
	...
	case 0x00://VPD page code = supported VPD pages
	  ...
	  USB->EP1R = (1<<8)|(1<<7)|(1<<0);//respond to OUT packets with NAK, ignore IN packets, clear CTR_RX flag
	  USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<7)|(1<<4)|(2<<0);//respond to IN packets with data, ingore OUT packets
...
static void processReadCapacityCommand_10()
{
  unsigned int sendLastLBA = 197119;//last accessible LBA is 197119
...
static void processReadCommand_10()
{
  if(MSDinfo.EjectFlag)//if medium is ejected
  ...
  else//if medium is available
    {
      ...
      //if anything in specified address range is not accessible
      if( (MSDinfo.DataPointer + MSDinfo.BytesLeft) > (197120 * 512) )
	{
	  ...
	  if(MSDinfo.BytesLeft == 0)//if host requested 0 blocks to be read
	    {
	      ...
	      USB->EP1R = (1<<8)|(1<<7)|(1<<0);//respond with NAK to OUT packets, ignore IN packets, clear CTR_RX flag
	      USB->EP2R = (1<<15)|(1<<14)|(1<<8)|(1<<7)|(1<<4)|(2<<0);//respond to IN packets with CSW, ignore OUT packets
	    }
...
static void sendResponse(void* responseAddress, unsigned int responseSize)
{
  ...
  if(USB->EP2R & (1<<14)) {bufferOffset = BTABLE->ADDR2_RX; countPointer = (unsigned short*) (&BTABLE->COUNT2_RX);}
  else                    {bufferOffset = BTABLE->ADDR2_TX; countPointer = (unsigned short*) (&BTABLE->COUNT2_TX);}
...
static void sendCSW(unsigned char status, unsigned char senseKey, unsigned char additionalSenseCode)
{
  ...
  bufferCopy( (unsigned short*) &(MSDinfo.CSW), (unsigned short*) (BTABLE_BaseAddr + BTABLE->ADDR2_TX), 13 );//copy CSW from RAM to PMA
  bufferCopy( (unsigned short*) &(MSDinfo.CSW), (unsigned short*) (BTABLE_BaseAddr + BTABLE->ADDR2_RX), 13 );//copy CSW from RAM to PMA
  BTABLE->COUNT2_TX = 13;
  BTABLE->COUNT2_RX = 13;
